...
public class EnableSoundGreeting  extends ListenerAdapter implements ICommand {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        ...
        ServiceCommand.getGuildConfig(event.getGuild().getIdLong()).soundGreeting = args[1].equals("+");
        EmbedBuilder msg = Messages.createEnableMsg("Звуковые приветствия " + ( (args[1].equals("+"))? "включены" : "отключены" ) );
        event.getChannel().sendMessage(msg.build()).queue();
    ...
    public String getDescriptions() {
        return "Включает/Отключает звуковые приветствия";
    }
